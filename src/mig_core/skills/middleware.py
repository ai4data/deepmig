...
1. Bundled skills: Shipped with the package (lowest precedence)
2. User-level: ~/.deepagents/{AGENT_NAME}/skills/
3. Project-level: {PROJECT_ROOT}/.deepagents/skills/ (highest precedence)
...
from mig_core.skills.load import SkillMetadata, list_skills
...
class SkillsMiddleware(AgentMiddleware):
    ...
    Supports three levels of skills (in order of precedence):
    - Bundled skills: Shipped with the package (defaults)
    - User skills: ~/.deepagents/{AGENT_NAME}/skills/ (override bundled)
    - Project skills: {PROJECT_ROOT}/.deepagents/skills/ (override all)
    ...
    def _format_skills_locations(self) -> str:
        ...
        if self.bundled_skills_dir:
            locations.append("**Bundled Skills**: Shipped with DeepMig (defaults)")
        locations.append(f"**User Skills**: `{self.user_skills_display}` (override bundled)")
        if self.project_skills_dir:
            locations.append(
                f"**Project Skills**: `{self.project_skills_dir}` (override all)"
            )
        return "\n".join(locations)
    ...
    def before_agent(self, state: SkillsState, runtime: Runtime) -> SkillsStateUpdate | None:
        ...
        bundled, user-level, and project-level directories.
